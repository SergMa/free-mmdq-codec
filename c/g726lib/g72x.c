...
//#ifdef abs
//#undef abs
#define abs(val) __builtin_bfin_abs_fr1x32(val)
//#endif

#define quan2(val)  ((val<=0)?(0):(31-__builtin_bfin_norm_fr1x32(val))&0xF)

//int quan2( volatile int val )
//{
//    int signbits;
//
//    if(val<=0)
	...
	g726_state *state_ptr)
{
    ...
    //sezi  = fmult(state_ptr->b[0] >> 2, state_ptr->dq[0]);
    //sezi += fmult(state_ptr->b[1] >> 2, state_ptr->dq[1]);
    //sezi += fmult(state_ptr->b[2] >> 2, state_ptr->dq[2]);
    //sezi += fmult(state_ptr->b[3] >> 2, state_ptr->dq[3]);
    //sezi += fmult(state_ptr->b[4] >> 2, state_ptr->dq[4]);
    //sezi += fmult(state_ptr->b[5] >> 2, state_ptr->dq[5]);
	...
	g726_state *state_ptr)	/* coder state pointer */
{
	...
	if (tr == 1) {			/* reset a's and b's for modem signal */
        ...
        if( code_size != 5 ) { /* for G.721 and 24Kbps G.723 */
            if (dq & 0x7FFF) {
                for (cnt = 0; cnt < 6; cnt++) {
                    state_ptr->b[cnt] -= state_ptr->b[cnt] >> 8;
                    if ((dq ^ state_ptr->dq[cnt]) >= 0)
